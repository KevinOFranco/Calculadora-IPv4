...
function calcularResultado() {
    if(validarIp(primerOctante.value, segundoOctante.value, tercerOctante.value, cuartoOctante.value, mascaraSubred.value)) {
        ...
        document.getElementById('direccionRedRespuesta').innerHTML = direccionRed;
        ...
        let totalHostsRespuesta = calcularTotalHosts(mascaraSubred.value);
        document.getElementById('totalHostsRespuesta').innerHTML = totalHostsRespuesta;
        ...
        if(parseInt(mascaraSubred.value, 10) < 31) {
            ...
            document.getElementById('direccionHostFinTabla').innerHTML = calcularDireccionHostEnPosicion(totalHostsRespuesta, direccionRed); //Encuentra la dirección del último host.

            llenarListadoDirecciones(direccionRed, totalHostsRespuesta);
        } else {
...
const calcularTotalHosts = (tamanioMascara) => {
    let bitsHost = TAMANIO_DIRECCION - parseInt(tamanioMascara, 10);
...
const limpiarCampos = () => {
    ...
    document.getElementById('direccionRedRespuesta').innerHTML = '';
    document.getElementById('broadcastRespuesta').innerHTML = '';
    document.getElementById('totalHostsRespuesta').innerHTML = '';
